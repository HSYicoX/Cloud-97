    ...
    <CardContent className="p-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
        ...
        <div>
          <Select value={selectedCategory} onValueChange={onCategoryChange}>
            ...
            <SelectContent position="popper" className="hidden data-[state=open]:block data-[state=closed]:hidden">
              <SelectItem value="all">所有分类</SelectItem>
              {categories.map(category => <SelectItem key={`cat-${category}`} value={category}>
                {category}
              </SelectItem>)}
            </SelectContent>
          </Select>
      ...
      <div className="border-t border-slate-700/50 pt-4">
        ...
        {allTags.length > 0 ? <div className="flex flex-wrap gap-2">
          {allTags.slice(0, showTagFilter ? allTags.length : 8).map(tag => <RippleEffect key={`tag-${tag}`}>
          ...
          {allTags.length > 8 && <RippleEffect>
            <Badge variant="outline" className="cursor-pointer bg-slate-700/50 text-slate-300 border-slate-600 hover:bg-slate-600/50" onClick={() => onToggleTagFilter(!showTagFilter)}>
